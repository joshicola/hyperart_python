...
class Point:
    ...
    def transform(self, t: Transformation):
        ...
        Args:
            t (Transformation): _description_
        """
...
def weierstrassCrossProduct(p1: Point, p2: Point) -> Point:
    ...
    if p1.type() == 0 and p2.type() == 0:
        return Point(0, 0)
    elif p1.type() == 1 and p2.type() == 1:
        return Point(p1.w() * p2.w(), 0)
    elif p1.type() == 0 and p2.type() == 1:
        return Point(p2.w(), 0)
    elif p1.type() == 1 and p2.type() == 0:
        return Point(p1.w(), 0)
    else:
        return Point(0, 0)
...
class Element:
    ...
    def setLineStyle(self, ls: int):
        self._lineStyle = ls

    def clone(self):
        e = copy.deepcopy(self)
        # TODO: need id factory for this
        e._id = 5
        return e
...
class Circle(Element):
    def __init__(self):
        super().__init__()
        # TODO: Check on Enumerated Types in Python
...
#     If shouldDrawArc is false, then hyperline should be approximated using a straight
...
class HyperPoly(HyperPolyLine):
    ...
    def transform(self, tran):
        for i in range(len(self.points_)):
            self.points_[i] = tran.apply(self.points_[i])
